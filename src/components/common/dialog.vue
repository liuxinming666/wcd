  ...
  <transition name="dialog">
    <div class="dialog-wrap" v-show='showWin' @mousedown='down' @mouseup='up' @mousemove='move' ref='dialogArea'>
      ...
      <div class="dialog" ref='dialog' :style='{top:top+"px",left:left+"px",width:curWidth+"px",height:curHeight+"px"}'>
        ...
        <div class="submit" @click='submit' v-if="bShowSubmitBtn">确定</div>
...
export default {
  props:{
    ...
    height:{
      ...
      devX:{//X方向的偏移量，主要用于窗口最大化时
          type:[String,Number],
          default:0
      },
      devY:{//Y方向的偏移量，主要用于窗口最大化时
          type:[String,Number],
          default:0
      },
      bShowSubmitBtn:{
  ...
  methods: {
    ...
    maxWin(){
      this.max=!this.max;
      if(this.max){
        var h=this.$refs.dialogArea.offsetHeight - this.devY;
